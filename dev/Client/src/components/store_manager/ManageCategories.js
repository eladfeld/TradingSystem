...
export default function ManageCategories({ getAppState, setAppState }) {
  ...
  const addCat = async () => {
    ...
    if (category !== "") {
      axios
        .post(`${SERVER_BASE_URL}addCategoryToRoot`, {
          userId,
          storeId,
          category,
        })
      ...
      axios
        .post(`${SERVER_BASE_URL}addCategory`, {
          userId,
          storeId,
          categoryFather: main_cat,
          category: sub_cat,
        })
        .then((res) => {
          if (res.status == 200) {
        ...
        <div>
          {_isSucsess ? (
            ...
            <Alert severity="warning">
              A problem accured while adding the category: {problem}!
            </Alert>
          ) : (
            <Grid>
