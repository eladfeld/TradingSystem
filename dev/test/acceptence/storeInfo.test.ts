...
describe('store info test' , function() {

    it('store info test good' , function() {
        //TODO: change this test when getStoreInfo returns json
        ...
        if (isOk(enter_res))
        {
            ...
            if(isOk(login_res))
            {
                ...
                if (isOk(store1) && isOk(store2))
                {
                    ...
                    console.log(JSON.stringify(service.getStoreInfo(subscriber.getUserId() , store1.value.getStoreId()),null,2));
                }
            }
            else{
                console.log("fail");
    ...
    it('store info test bad' , function() {
        //TODO: change this test when getStoreInfo returns json
        Authentication.clean();
        let service: Service = Service.get_instance();
        service.clear();
        ...
        if (isOk(enter_res))
        {
            ...
            if(isOk(login_res))
            {
                ...
                if (isOk(store1) && isOk(store2))
                {
                    service.addNewProduct(subscriber.getUserId() , store1.value.getStoreId() , "Apple" , [Category.SHIRT] , 26 , 10);
                    service.addNewProduct(subscriber.getUserId(), store2.value.getStoreId() , "banana" , [Category.SHIRT] , 26 , 20);
                    console.log(JSON.stringify(service.getStoreInfo(subscriber.getUserId() + 1 , store1.value.getStoreId()),null,2));
                }
            }
            else{
                console.log("fail");
    ...
    it('avi opens store , system manager watching it' , function() {
        ...
        if(isOk(enter_res))
        {
            ...
            if (isOk(login_res))
            {
                let subscriber: Subscriber = login_res.value;
                if (isOk(store1))
                    console.log(service.getStoreInfo(subscriber.getUserId() , store1.value.getStoreId()));
            }
        }
        //---------------------------------------------------------------
        assert.fail()
