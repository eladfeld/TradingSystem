...
describe('edit store inventory (use case 4.1)' , function() {
        ...
        if (isOk(enter_res))
        {
            ...
            if(isOk(login_res))
            {
                ...
                if(isOk(storeRes))
                {
                    ...
                    if (isOk(productId))
                    {
                        console.log(service.editStoreInventory(subscriber.getUserId(), storeRes.value.getStoreId(), productId.value, 10));
                        console.log(service.getStoreInfo(subscriber.getUserId(), storeRes.value.getStoreId()));
                    }
                    else assert.fail();
                }
                else assert.fail();
